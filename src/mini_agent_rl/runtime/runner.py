...
class AgentRunner:
    def __init__(self, model: ModelClient, tools: ToolRegistry, reward: RewardFunction, store: SQLiteStore, max_steps: int = 5): self.model, self.tools, self.reward, self.store, self.max_steps = model, tools, reward, store, max_steps
    async def run_rollout(self, task: Task, group_id: str, seed: int = 0) -> Rollout:
        ...
        try:
            for step in range(self.max_steps):
                ...
                if response.tool_call:
                    ...
                    transition = Transition(rollout_id=rollout.id, step=step, messages=messages.copy(), response=response, observation=observation); messages += [Message(role="assistant", content=response.content), Message(role="tool", content=str(observation))]
        ...
        except asyncio.CancelledError: rollout.status=RolloutStatus.CANCELLED; rollout.error="任务被取消"; raise
        ...
        rollout.finished_at=now(); self.store.save_rollout(rollout); return rollout

    async def run_group(self, task: Task, group_size: int = 4) -> RolloutGroup:
        group=RolloutGroup(task_id=task.id); self.store.save_task(task); self.store.save_group(group); results=await asyncio.gather(*(self.run_rollout(task, group.id, i) for i in range(group_size)), return_exceptions=False); group.rollout_ids=[r.id for r in results]; group.rewards=[r.reward for r in results]; group.mean_reward=statistics.mean(group.rewards) if group.rewards else 0.0; group.std_reward=statistics.pstdev(group.rewards) if len(group.rewards)>1 else 0.0; group.success_rate=sum(r.status == RolloutStatus.SUCCEEDED for r in results)/len(results) if results else 0.0; self.store.save_group(group); return group
