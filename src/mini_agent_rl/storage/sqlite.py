...
class SQLiteStore:
    ...
    def init(self):
        self.conn.executescript("""CREATE TABLE IF NOT EXISTS schema_version(version INTEGER NOT NULL); INSERT INTO schema_version SELECT 1 WHERE NOT EXISTS (SELECT 1 FROM schema_version); CREATE TABLE IF NOT EXISTS tasks(id TEXT PRIMARY KEY, data TEXT NOT NULL); CREATE TABLE IF NOT EXISTS rollout_groups(id TEXT PRIMARY KEY, data TEXT NOT NULL); CREATE TABLE IF NOT EXISTS rollouts(id TEXT PRIMARY KEY, group_id TEXT NOT NULL, status TEXT NOT NULL, data TEXT NOT NULL); CREATE TABLE IF NOT EXISTS transitions(id TEXT PRIMARY KEY, rollout_id TEXT NOT NULL, step INTEGER NOT NULL, data TEXT NOT NULL); CREATE TABLE IF NOT EXISTS events(id TEXT PRIMARY KEY, rollout_id TEXT NOT NULL, type TEXT NOT NULL, data TEXT NOT NULL); UPDATE schema_version SET version=2;"""); self.conn.commit()
    ...
    def get_transitions(self, rollout_id: str) -> list[Transition]:
        return [Transition.model_validate_json(row[0]) for row in self.conn.execute("SELECT data FROM transitions WHERE rollout_id=? ORDER BY step", (rollout_id,))]
    ...
    def get_group(self, group_id: str):
        row = self.conn.execute("SELECT data FROM rollout_groups WHERE id=?", (group_id,)).fetchone(); return RolloutGroup.model_validate_json(row[0]) if row else None
    ...
    def export_jsonl(self, path: str):
        ...
        with open(path, "w", encoding="utf-8") as f:
            for row in self.conn.execute("SELECT data FROM rollouts"):
                rollout = Rollout.model_validate_json(row[0]); transitions = [Transition.model_validate_json(x[0]).model_dump(mode="json") for x in self.conn.execute("SELECT data FROM transitions WHERE rollout_id=? ORDER BY step", (rollout.id,))]; group = self.get_group(rollout.group_id); f.write(json.dumps({"group": group.model_dump(mode="json") if group else None, "rollout": rollout.model_dump(mode="json"), "transitions": transitions}, ensure_ascii=False) + "\n")
